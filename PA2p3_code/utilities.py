...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            # Save the plot
            plt.savefig(f"attention_map_{j + 1}_after_{sentence[0]}.png")
            
            # Show the plot
            # plt.show()
            
    def decode_sanity_check(self, sentence, block_size, predictions):
        # Encode the sentence using the tokenizer
        wordids = self.tokenizer.encode(sentence)
        ...
        # Process the input tensor through the encoder model
        _, attn_maps = self.model(input_tensor, predictions) # Ignore the output of the model, and only get the attention maps; make sure your encoder returns the attention maps
